  ...
  const handleOAuthLogin = useCallback(
    async (provider: 'google' | 'discord') => {
      ...
      try {
        ...
        const authorizationUrl = await authService.startOAuth(provider, deviceFingerprint)
        window.location.href = authorizationUrl
      } catch (error) {
        toast({
          title: 'Erro',
          description: getApiErrorMessage(error, `Falha ao iniciar login com ${provider === 'google' ? 'Google' : 'Discord'}`),
          variant: 'destructive',
        })
        setIsLoading(false)
      }
          ...
          <div className="relative overflow-hidden rounded-[1.75rem] border border-white/[0.08] bg-gradient-to-b from-white/[0.05] to-white/[0.01] p-6 shadow-[0_8px_50px_rgba(0,0,0,0.7),0_0_80px_rgba(220,38,38,0.05),inset_0_1px_0_rgba(255,255,255,0.06)] sm:rounded-[2rem] sm:p-10">
            ...
            <div className="relative">
              <div className="mb-8 text-center">
                ...
                <p className="mt-2 text-sm text-gray-500">{mode === 'login' ? 'Entre na sua conta' : 'Crie sua conta'}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] py-3 transition hover:border-white/[0.14] hover:bg-white/[0.05] disabled:cursor-not-allowed disabled:opacity-60"
                >
                ...
                >
                  ...
                  <span className="text-xs font-medium text-gray-200">Discord</span>
                </button>
              </div>
