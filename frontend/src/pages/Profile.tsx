...
export function Profile() {
  ...
  const handleActivate = async () => {
    if (!licenseKey.trim()) {
      toast({ title: 'Erro', description: 'Digite uma chave de licença', variant: 'destructive' })
      return
    }
    ...
    try {
      ...
      setLicenseKey('')
      toast({ title: 'Licença ativada', description: response.message })
    } catch (error) {
      toast({ title: 'Erro', description: getApiErrorMessage(error, 'Falha ao ativar licença'), variant: 'destructive' })
    } finally {
      setActivating(false)
    }
  }
    ...
    <div className="mx-auto max-w-[800px] p-4 sm:p-6 lg:p-8">
      ...
      >
        <div className="mb-1 flex items-center gap-2.5">
          ...
          <h2 className="text-lg font-bold text-white">Licença</h2>
        </div>
        <p className="mb-6 ml-[30px] text-xs text-gray-500">Ative a chave comprada para liberar a plataforma</p>
        ...
        <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-3.5 shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)]">
            ...
            <span className="text-sm font-semibold text-white">{user?.licensePlan || 'Não ativado'}</span>
          </div>
        </div>
