...
export const loraSchema = z.object({
  ...
  triggerWord: z.string().min(2, 'Palavra-chave e obrigatoria'),
...
export const faceSwapSchema = z.object({
  sourceImageUrl: z.string().url('URL da imagem de origem invalida'),
  targetImageUrl: z.string().url('URL da imagem de destino invalida'),
  loraStrength: z.number().min(0).max(1),
})

export const videoMotionSchema = z.object({
  imagePrompt: z.string().min(5, 'Prompt da imagem e obrigatorio'),
